...
function Manage() {
  ...
  const load = async () => {
    setLoading(true);
    const data = (await getDeployments()) as Deployment[];
    setItems(
  ...
  const action = async (
    id: string,
    status: "running" | "stopped" | "building" | "queued" | "failed" | "crashed",
    msg: string,
  ) => {
    await updateDeployment({ id, updates: { status, status_message: msg } });
    toast.success(msg);
    load();
  };
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="glass-card p-8 text-center text-xs text-muted-foreground">
          No deployments yet.{" "}
          <Link to="/app/deploy" className="text-primary">
            ...
            <div key={d.id} className="glass-card p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap gap-1 shrink-0">
                  {d.status !== "running" && (
                    <IconBtn label="Start" onClick={() => action(d.id, "running", "Started")}>
                      <Play className="h-3 w-3" />
                    </IconBtn>
                  )}
                  {d.status === "running" && (
                    <IconBtn label="Stop" onClick={() => action(d.id, "stopped", "Stopped")}>
                      <Square className="h-3 w-3" />
                    </IconBtn>
                  )}
                  <IconBtn
                    label="Restart"
                    onClick={() => action(d.id, "building", "Restarting...")}
                  >
                    <RotateCw className="h-3 w-3" />
                  </IconBtn>
                  <IconBtn label="Logs" onClick={() => setSelected(d)}>
                    <FileText className="h-3 w-3" />
                  </IconBtn>
                  <IconBtn label="Delete" onClick={() => del(d.id)} destructive>
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[10px] px-1.5 py-0.5 rounded border ${map[status] ?? map.stopped}`}>
      {status}
    </span>
  );
}
...
function LogsDrawer({ deployment, onClose }: { deployment: Deployment; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      const data = (await getDeploymentLogs(deployment.id)) as Record<string, unknown>[];
      if (mounted)
        setLogs(
