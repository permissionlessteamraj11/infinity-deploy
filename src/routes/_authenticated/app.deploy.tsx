    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Deploy from GitHub or upload a ZIP file.
        </p>
      </div>
      <Tabs defaultValue="github">
        <TabsList className="grid grid-cols-2 h-9">
          <TabsTrigger value="github" className="text-xs gap-1.5">
            <Github className="h-3.5 w-3.5" /> From GitHub
          </TabsTrigger>
          <TabsTrigger value="zip" className="text-xs gap-1.5">
            <FileArchive className="h-3.5 w-3.5" /> Upload ZIP
          </TabsTrigger>
        </TabsList>
...
function GithubForm() {
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        const dep = (await createDeployment({
          ...
          port: Number(port),
          status: "queued",
          status_message: "Queued for build",
        })) as Record<string, unknown>;

        if (!dep) {
          setLoading(false);
          return toast.error("Failed to create deployment");
        }

        if (envs.length) {
          await addDeploymentEnvs(
            envs
              .filter((e) => e.k)
              .map((e) => ({
                deployment_id: dep.id as string,
                user_id: u.user!.id,
                key: e.k,
                value: e.v,
              })),
        ...
        await simulateDeploy(dep.id as string, u.user.id, { source: "github", repo });
    ...
    >
      ...
      <Field label="Branch">
        <Input value={branch} onChange={(e) => setBranch(e.target.value)} className="h-9 text-xs" />
      </Field>
      ...
      <Field
        label="Start command"
        hint="Leave empty for auto-detect (main.py / app.py / package.json start)"
      >
        <Input
          value={start}
        ...
        <div className="space-y-3 pl-3 border-l-2 border-primary/30">
          <Field label="Runtime">
            <Input
              ...
              className="h-9 text-xs"
              placeholder="auto, python3.11, node20"
            />
          </Field>
          <Field label="Port">
            <Input
...
function ZipForm() {
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        const dep = (await createDeployment({
          ...
          deploy_cmd: start || null,
          status: "queued",
          status_message: "ZIP received, queued for build",
        })) as Record<string, unknown>;
        ...
        await simulateDeploy(dep.id as string, u.user.id, {
          source: "zip",
          filename: file.name,
        });
    ...
    >
      ...
      <Field label="ZIP file (max 100MB)" required>
        <label className="glass-card p-6 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-primary/50 transition-colors">
          ...
          <span className="text-xs">
            {file ? file.name : "Click to select or drop a .zip file"}
          </span>
          <input
            type="file"
      ...
      <Field
        label="Build command"
        hint="Auto-detected if empty. Single .py file → python3 <file>.py"
      >
        <Input
          value={build}
          onChange={(e) => setBuild(e.target.value)}
...
function EnvEditor({
  ...
  setEnvs: (e: { k: string; v: string }[]) => void;
}) {
  return (
    <div>
...
async function simulateDeploy(
  depId: string,
  userId: string,
  meta: { source: string; repo?: string; filename?: string },
) {
  ...
  await log(
    `[BUILD] Preparing environment for ${meta.source === "github" ? meta.repo : meta.filename}...`,
  );
  await updateDeployment({
    id: depId,
    updates: { status: "building", status_message: "Building" },
  });
  ...
  await updateDeployment({
    id: depId,
    updates: { status: "running", status_message: "Running" },
  });
