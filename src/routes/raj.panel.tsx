        ...
        <div className="container-app py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to="/" className="text-xs text-muted-foreground hover:text-foreground">
              <ArrowLeft className="h-3 w-3 inline" /> Exit
            </Link>
            <span className="text-sm font-semibold">Admin Panel</span>
          </div>
          <span className="text-[10px] px-2 py-0.5 rounded bg-destructive/15 text-destructive border border-destructive/30">
            RESTRICTED
          </span>
        </div>
...
function UsersPanel() {
  ...
  const load = async () => {
    ...
    const p = (await getProfiles()) as any[];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const w = (await getWallets()) as any[];
    setUsers(p ?? []);
  ...
  const ban = async (id: string, banned: boolean) => {
    await updateProfile({ id, updates: { is_banned: !banned } });
          ...
          <div key={u.id} className="p-4 flex flex-wrap items-center gap-3 justify-between">
            <div className="min-w-0">
              <div className="text-sm font-medium">
                {u.full_name ?? "Unnamed"}{" "}
                {u.is_banned && <span className="text-[10px] text-destructive ml-1">BANNED</span>}
              </div>
              <div className="text-[11px] text-muted-foreground">
                {u.email} · code: {u.referral_code}
              </div>
              <div className="text-[11px] text-muted-foreground">
                Balance: ₹{(wallets[u.id] ?? 0).toFixed(2)}
              </div>
            </div>
...
function DeploysPanel() {
  ...
  const load = async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data = (await getDeployments()) as any[];
    setDeps(data ?? []);
  };
  ...
  const ban = async (id: string, banned: boolean) => {
    await updateDeployment({
      id,
      updates: { is_banned: !banned, status: !banned ? "stopped" : "queued" },
    });
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold">All deployments ({deps.length})</h2>
      </div>
      <div className="glass-card divide-y divide-border">
        {deps.map((d) => (
          <div key={d.id} className="p-4 flex flex-wrap items-center gap-3 justify-between">
            <div className="min-w-0">
              <div className="text-sm font-medium">
                {d.name} <span className="text-[10px] text-muted-foreground">[{d.status}]</span>
              </div>
              <div className="text-[11px] text-muted-foreground truncate">
                {d.source_type}: {d.repo_url ?? "ZIP"}
              </div>
              <div className="text-[10px] text-muted-foreground font-mono">
                owner: {d.user_id.slice(0, 8)}
              </div>
            </div>
            <Button
              size="sm"
...
function ChatsPanel() {
  ...
  const load = async () => {
    ...
    (data ?? []).forEach((c) => {
      if (!grouped[c.user_id])
        grouped[c.user_id] = { user_id: c.user_id, last: c.message, unread: 0 };
  ...
  useEffect(() => {
    ...
    (async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const data = (await getChatsForUser(openUser)) as any[];
      setMsgs(data ?? []);
    })();
  }, [openUser]);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-semibold">Chats ({threads.length})</h2>
      </div>
        ...
        <div className="glass-card divide-y divide-border max-h-[70vh] overflow-auto">
          {threads.map((t) => (
            ...
            >
              ...
              {t.unread > 0 && (
                <span className="text-[10px] bg-primary text-primary-foreground rounded-full px-1.5 mt-1 inline-block">
                  {t.unread} new
                </span>
              )}
            </button>
            ...
            <>
              <div className="flex-1 overflow-auto p-4 space-y-2">
                {msgs.map((m) => (
                  ...
                  >
                    <div
                      className={`max-w-[75%] text-xs px-3 py-2 rounded-lg ${m.sender === "admin" ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                    >
                      {m.message}
                    </div>
                  </div>
